...
public class FileManager {
    ...
    // ... (FileItem and FileListing inner classes are unchanged)
    public static class FileItem {
        public final String name; public final String type; public final String uri;
        FileItem(String name, String type, String uri) { this.name = name; this.type = type; this.uri = uri; }
    }
    public static class FileListing {
        public final List<FileItem> items; public final String currentPath; public final String parentUri; public final String error;
        FileListing(List<FileItem> items, String currentPath, String parentUri, String error) { this.items = items; this.currentPath = currentPath; this.parentUri = parentUri; this.error = error; }
    }
    ...
    public static FileListing listFiles(Context context, Uri directoryUri) {
        ...
        } catch (Exception e) { return new FileListing(Collections.emptyList(), currentPath, null, "Error: " + e.getMessage()); }
    ...
    // ... (All other helper methods like downloadFile, uploadFile, etc., are unchanged)
    ...
    public static String getPathFromUri(Context context, Uri uri) { if (!DocumentsContract.isTreeUri(uri)) return "/"; List<String> pathSegments = new ArrayList<>(); Uri currentUri = uri; while (true) { String segmentName = getFileNameFromUri(context, currentUri); pathSegments.add(segmentName); Uri parentUri = getParentUri(context, currentUri); if (parentUri == null || parentUri.equals(currentUri)) { break; } currentUri = parentUri; } Collections.reverse(pathSegments); return "/" + String.join("/", pathSegments); }
    private static Uri getParentUri(Context context, Uri folderUri) { try { Uri documentUri = DocumentsContract.buildDocumentUriUsingTree(folderUri, DocumentsContract.getDocumentId(folderUri)); try (Cursor c = context.getContentResolver().query(documentUri, new String[]{DocumentsContract.Document.COLUMN_PARENT_DOCUMENT_ID}, null, null, null)) { if (c != null && c.moveToFirst()) { String parentDocId = c.getString(0); if (parentDocId != null) { return DocumentsContract.buildDocumentUriUsingTree(folderUri, parentDocId); } } } } catch (Exception e) { /* Failsafe */ } return null; }
}